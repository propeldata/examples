...
export default function SSR({ propelData }) {   
    ...
    const formatHeader = (header) => header.split('_').map(word =>
        word.charAt(0).toUpperCase() + word.slice(1).toLocaleLowerCase()
    ).join(' ')
    // Prepare to format the leaderboard value data as US Dollars, without cents:
    const formatOptions = { style: "currency", currency: "USD", minimumFractionDigits: 0 };
    // Grab the Intl.NumberFormat helper to format numbers with commas:
    const internationalNumberFormat = new Intl.NumberFormat("en-us",formatOptions)
            ...
            <Row style={{padding:5}}>
                ...
                <Col md={6} style={{display:'flex', justifyContent:'right', padding:10, backgroundColor: '#464646'}}>
                    <Dropdown>
                        ...
                        <Dropdown.Menu >
                            <Dropdown.Item href="/leaderboard?timeRange=YESTERDAY">Yesterday</Dropdown.Item>
                            <Dropdown.Item href="/leaderboard?timeRange=LAST_7_DAYS">Last 7 days</Dropdown.Item>
                            <Dropdown.Item href="/leaderboard?timeRange=LAST_30_DAYS">Last 30 days</Dropdown.Item>
                            <Dropdown.Item href="/leaderboard?timeRange=LAST_90_DAYS">Last 90 days</Dropdown.Item>
                            <Dropdown.Item href="/leaderboard?timeRange=THIS_YEAR">This year</Dropdown.Item>
                            <Dropdown.Item href="/leaderboard?timeRange=LAST_YEAR">Last year</Dropdown.Item>
                            <Dropdown.Item href="/leaderboard?timeRange=LAST_2_YEARS">Last 2 years</Dropdown.Item>
                            </Dropdown.Menu>
                    </Dropdown>
                </Col>
            </Row>
            <Row>
                ...
                <Table striped bordered hover size="sm">
                    <thead>
                        ...
                        {headers.map((header) =>
                            (<th key={header}>
                                {header === "value" ? "Category Sales" : formatHeader(header)}
                            </th>)
                        )}
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        {rows[0].map((_, index) => (
                            <tr key={`${rows[0][index]}${rows[1][index]}${index}`}>
                            <td>{index + 1}</td>
                            {/* Access each column directly and create a cell for each data point: */}
                            <td>{rows[0][index]}</td>
                            <td>{formatDollars(rows[1][index])}</td>
                            </tr>
                        ))}
                    </tbody>
                </Table>
...
export async function getServerSideProps(context) {
    ...
    const timeSeriesQuery = gql`
        query timeSeries ($uniqueName: String!, $timeRange: TimeRangeInput!, $rowLimit: Int!, $sort: Sort, $dimensions: [DimensionInput!]!) {
